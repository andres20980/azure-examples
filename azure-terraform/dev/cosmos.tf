...
resource "azurerm_private_dns_zone_virtual_network_link" "cosmos_db" {
  name                  = "cosmosdb-dev-private-dns-zone-link"
  resource_group_name   = azurerm_resource_group.rg.name
...
resource "azurerm_private_endpoint" "cosmos_db" {
  name                          = "${var.environment}-sacyrgpt-cosmosdb-plink"
  location                      = var.location
  resource_group_name           = azurerm_resource_group.rg-sacyrgpt.name
  subnet_id                     = azurerm_subnet.aks_subnet_new.id
  custom_network_interface_name = "${var.environment}-sacyrgpt-cosmosdb-plink-nic"
  tags                          = var.tags_ia

  private_service_connection {
    name                           = "${var.environment}-sacyrgpt-cosmosdb-plink"
    is_manual_connection           = false
    private_connection_resource_id = azurerm_cosmosdb_account.sacyrgpt-cosmos.id
