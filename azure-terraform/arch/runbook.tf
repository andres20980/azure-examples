...
resource "azurerm_automation_schedule" "monday_friday_morning" {
  ...
  start_time              = "${local.current_wallclock_time >= "6.50" ? local.tomorrow : local.today}T05:55:00Z" # Azure runs it in UTC
...
resource "azurerm_automation_schedule" "monday_friday_evening" {
  ...
  interval                = 1
  timezone                = "Europe/Madrid"
  start_time              = "${local.current_wallclock_time >= "21.50" ? local.tomorrow : local.today}T20:55:00Z" # Azure runs it in UTC
  week_days               = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]
}
